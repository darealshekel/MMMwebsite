  ...
  return (
    <nav className="fixed top-0 left-0 right-0 z-50 glass-panel-strong border-b border-border/40">
      <div className="container mx-auto flex items-center justify-between h-16 px-4">
        <Link to="/" className="interactive-tab flex items-center gap-2.5 group">
        ...
        <div className="hidden md:flex items-center gap-1">
          {navLinks.map((l) => (
            <Link
              key={l.to}
              to={l.to}
              className={`interactive-tab px-3.5 py-2 rounded-lg text-sm font-medium transition-colors ${
            ...
            <div className="p-4 space-y-2">
              {navLinks.map((l) => (
                <Link
                  ...
                  className="interactive-tab block px-4 py-2.5 rounded-lg text-sm font-medium text-muted-foreground hover:text-foreground hover:bg-secondary/50 transition-colors"
                >
                  {l.label}
                </Link>
              ))}
