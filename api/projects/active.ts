...
export default async function handler(request: Request) {
  const allowed = await rateLimitRequest(request, "projects", "active", 120, 5 * 60 * 1000);
  ...
  return jsonResponse({ projects: snapshot.projects.filter((project) => project.isActive) });
}
