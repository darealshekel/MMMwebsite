import { lookupRoleByUuid, setRoleByUuid, AdminActionError } from "../_lib/admin-management.js";
...
export default async function handler(request: Request) {
  try {
    ...
    if (request.method === "GET") {
      ...
      return response(await lookupRoleByUuid(auth, uuid));
    }
    ...
    logServerError("admin-roles failed", error);
    return response({ error: "Unable to manage roles." }, { status: 500 });
  }
}
